...
learn.add_argument('--lr', type=float, default=1e-3, help='initial learning rate [default: 0.0005]')
...
learn.add_argument('--max-norm', default=5, type=int, help='Norm cutoff to prevent explosion of gradients')
...
learn.add_argument('--dynamic-lr', action='store_true', default=True, help='Use dynamic learning schedule.')
learn.add_argument('--milestones', nargs='+', type=int, default=[3,6,9], help=' List of epoch indices. Must be increasing. Default:[5,10,15]')
...
cnn.add_argument('--fc-size', type=int, default=1024, help='hidden units used in fully connected layers') 
cnn.add_argument('--last-pooling-layer', type=str, default='max-pooling', help='comma-separated kernel size to use for convolution')
cnn.add_argument('--highway-num', type=int, default=1, help='number of highway layers')

# device
device = parser.add_argument_group('Device options')
device.add_argument('--num-workers', default=1, type=int, help='Number of workers used in data-loading')
device.add_argument('--cuda', action='store_true', default=use_cuda, help='enable the gpu' )
# experiment options
...
experiment.add_argument('--save-folder', default='checkpoints', help='Location to save epoch models, training configurations and results.')
...
def train(train_loader, dev_loader, model, args):
    ...
    if args.optimizer=='Adam':
        ...
        optimizer = torch.optim.Adadelta(model.parameters(), lr=args.lr, rho=0.95, eps=1e-5)
    ...
    for epoch in range(start_epoch, args.epochs+1):
        ...
        if best_acc is None or val_acc > best_acc:
            file_path = '%s/CNN_best.pth.tar' % (args.save_folder)
...
def main():
    ...
    # load training data
    print("\nLoading  data...")
    nclass, train_dataset, dev_dataset = get_train_val_data(label_data_path=args.train_path, 
                                    alphabet_path=args.alphabet_path, l0=args.l0, onehot=args.onehot)
    args.nclass = nclass
    print("Transferring  data into iterator...")
    ...
    print(model)
    print 'Parameter Numbers: {:4.2f} M'.format(get_model_param_num(model)/1e6)

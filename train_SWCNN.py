...
cnn.add_argument('--dropout', type=float, default=0, help='the probability for dropout [default: 0.5]')
...
cnn.add_argument('--onehot', action='store_true', default=True, help='use onehot enc for input, also off for CharCRNN' )
# device
...
experiment.add_argument('--save-folder', default='checkpoints/SWCNN', help='Location to save epoch models, training configurations and results.')
...
def train(train_loader, dev_loader, model, args):
    ...
    for epoch in range(start_epoch, args.epochs+1):
        ...
        if args.checkpoint and epoch % args.save_interval == 0:
            file_path = '%s/SWCNN_epoch_%d.pth.tar' % (args.save_folder, epoch)
        ...
        if best_acc is None or val_acc > best_acc:
            file_path = '%s/SWCNN_best.pth.tar' % (args.save_folder)
